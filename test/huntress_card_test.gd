...
func _build_ui() -> void:
	...
	panel.add_child(_header("Play a huntress card"))
	var card_grid := GridContainer.new()
	card_grid.columns = 2
	panel.add_child(card_grid)
	var ids : Array = []
	for id in GameDataLoader.card_repository:
		if GameDataLoader.card_repository[id].get("character_id", "") == "huntress":
			ids.append(id)
	ids.sort()
	for id in ids:
...
func _on_play_pressed(card_id : String) -> void:
	...
	var ctx := HuntressCardTestContext.new(self)
...
# Board verbs wired to the real combatants/companion/layout on screen; dice
# rolls and player choices are logged stubs, same style as every other test
# context in this project.
class HuntressCardTestContext extends BoardContext:
	var test
	...
	func heal_companion(amount : int, target = null) -> void:
		var who : Combatant = target if target != null else caster
		if who.companion == null:
			test._log("heal_companion(%d) — %s has no companion" % [amount, who.name])
			return
		who.companion.heal(amount)
		test._log("heal_companion(%d) -> %s at %d/%d HP" % [amount, who.companion.companion_name, who.companion.hp, who.companion.max_hp])
	...
	func roll_die() -> int:
		...
		test._log("roll_die() -> %d (%s)" % [value, Skill.symbol_for_value("huntress", value)])
		return value
